...
int vm_create_vcpu_arch(vm_t *vm, vm_vcpu_t *vcpu)
{
    ...
    /* For secondary vcpus, we have to create an entire thread */
        ...
        char vmm_thread_name[17];
        snprintf(vmm_thread_name, 17, "%s_vmmcore%d", get_instance_name(), vcpu->vcpu_id);
        char vmm_fault_thread_name[31];
        snprintf(vmm_fault_thread_name, 31, "%s_vmmcore%d:fault_handler",
                 get_instance_name(), vcpu->vcpu_id);
        ...
        config = thread_config_priority(config, 254);
        config = thread_config_mcp(config, 254);

        err = sel4utils_start_fault_handler(endpoint.cptr, vm->vka, &vm->mem.vmm_vspace, cspace, 0,
                                              vmm_thread_name, &fault_thread);
        seL4_TCB_SetPriority(fault_thread.tcb.cptr, tcb, 254);
        ...
        err = sel4utils_start_thread(&new_thread,
                                     (sel4utils_thread_entry_fn)&vcpu_run_secondary,
                                       vcpu, NULL, 0);
