                          ...
                          int size, seL4_Word *result)
{
    ...
    DPRINTF("Enter vm_hpet_mmio_readl at %" PRIx64 "\n", addr);
    ...
    if (index >= 0x100 && index <= 0x3ff) {
        ...
        DPRINTF("Index in range of TN Registers: 0x%x\n", index);
        DPRINTF("Switch argument: 0x%x\n", (addr - 0x100) % 0x20);
        ...
        DPRINTF("Index in range (0x100-0x3ff): 0x%x\n", index);
        switch (index) {
        case HPET_ID:
            *result = (seL4_Word)s->capability;
            DPRINTF("hpet_id: 0x%lx\n", *result);
            break;
        case HPET_PERIOD:
            *result = (seL4_Word)(s->capability >> 32);
            DPRINTF("hpet_period: 0x%lx\n", *result);
            break;
        case HPET_CFG:
            *result = (seL4_Word)s->config;
            DPRINTF("hpet_cfg: 0x%lx\n", *result);
            break;
        case HPET_CFG + 4:
            DPRINTF("invalid HPET_CFG + 4 %s\n", __func__);
            *result = 0;
            break;
        case HPET_COUNTER:
            if (hpet_enabled(s)) {
        ...
        case HPET_COUNTER + 4:
            ...
            DPRINTF("reading counter + 4  = 0x%lx\n", cur_tick);
...
int vm_create_hpet(vm_t *vm) {
    ...
    assert(NULL != vcpu);
    hpet_reservation = vm_reserve_memory_at (vm,
                                             HPET_BASE,
                                             HPET_LEN,
                                             hpet_fault_callback,
                                             &hpet_state);
